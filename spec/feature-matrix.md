...
| A close that states more than it is closing | OS7017, naming what was asked for and what is held: no order crosses zero, and a quantity the script wrote is a claim about its own position rather than a number the engine may quietly reduce | `specified` | `stdlib.md` 17.1, `stdlib.md` 17.2, `errors.md` OS7017 | `order/close-beyond-position` |
| Where that comparison is made | Only where the stated quantity and the folded position count the same thing, which is a declaration in units; in lots, cash or equity percent the two are different kinds of number and the call is not held to it | `specified` | `stdlib.md` 17.2, `host-interface.md` 7.1 | `order/close-qty-unit` |
