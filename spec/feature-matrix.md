...
| The orders one bar sends against one position | They can never sum past it: a position moves only when a fill settles, so every reducing order is measured against what is left after the bar's own earlier orders rather than against what the leg held when the bar began | `specified` | `stdlib.md` 17.1, `stdlib.md` 17.2 | `order/bar-reducing-budget` |
