...
| Overflowing flow span | A bar whose high to low span overflows has an absent flow term rather than a zero one, so the accumulation keeps its total and the window holding it is absent | `implemented` | `compiled-program.md` 3.1, `stdlib.md` 20.6 | `math/flow-span-overflow` |
| Negative window volume | `cmf` divides by a window volume that sums below zero like any other divisor and is absent only where the window volume is zero | `implemented` | `stdlib.md` 20.6 | `math/cmf-negative-volume` |
