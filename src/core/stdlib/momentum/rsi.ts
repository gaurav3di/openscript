 ...
 * changes and a change needs two bars, so the smoothing's window is bars 1 to
...
export function rsiTail(len = 14): Tail<Value, Value> {
  ...
  return {
    next(value: Value): Value {
      ...
      // No down bar in the window is the top of the scale. This also catches a
      // window that never moved at all, where both averages are zero and the
      // ratio has no value; every reference implementation reads 100 there, and
      // disagreeing with all of them on a flat window would be a worse answer
