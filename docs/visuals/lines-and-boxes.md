...
|---|---|
| `draw.setFrom(obj, t, p)` | A line's or box's first anchor |
| `draw.setTo(obj, t, p)` | Its second anchor |
| `draw.setBounds(obj, t1, p1, t2, p2)` | Both anchors in one call |
| `draw.setAt(label, t, p)` | A label's anchor |
| `draw.setPoints(polyline, times, prices)` | A polyline's whole path |
| `draw.setText(obj, text)` | A label's or box's caption |
| `draw.setColor(obj, color)` | The line or border colour |
| `draw.setTextColor(obj, color)` | The text colour |
| `draw.setFillColor(obj, color)` | A box's or polyline's fill |
| `draw.setWidth(obj, width)` | Line thickness |
| `draw.setStyle(obj, style)` | `"solid"`, `"dashed"` or `"dotted"` |
| `draw.setExtend(line, left, right)` | Whether the line continues to the pane edge |
| `draw.setTooltip(obj, text)` | The detail shown on hover |
| `draw.delete(obj)` | Removes one object |
